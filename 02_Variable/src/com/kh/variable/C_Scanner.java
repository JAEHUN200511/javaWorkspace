...
public class C_Scanner {
	...
	public void inputTest1() {
		...
		String lastName = sc.nextLine(); // [스페이스]동현
		// nextLine함수는 호출 후 "버퍼"의 값을 비워준다. => [엔터]값이 소멸
	...
	public void inputTest2() {
		...
		// 정수값을 뽑을 때 => sc.Int() / nextByte() / nextLong() / nextShort()
		...
		char gender = sc.nextLine().charAt(0); // charAt(0)은 입력한 문자에서 0번째 위치의 문자를 뽑아씀. 
		// "M"에서 0번째 위치의 문자 'M'
		// "male"에서 0번째 위치의 문자 => 'M'
		
		System.out.print(name+"님의 성별은 "+gender+" 입니다.");
		/*
		 * 주의사항
		 * 1) sc.nextXX() : 메서드들 뒤에 sc.nextLine()메서드가 와야 될 경우
		 *                  sc.nextLine()을 한번 더 호출하여 버퍼를 비워줘야 한다.
		 * 2) 스캐너를 통해 '문자'값을 입력 받아야 될 경우
		 *    sc.nextLine()을 통해 "문자열"을 입력받고, 그 후 charAt(인덱스)를 호출해야 한다.
		 */
